...
void HashTable<T1, T2>::set(T1 key, T2 value) {
	...
	for (int i = 0; i < pDataSet[setIndex].size(); i++) {
		if (pDataSet[setIndex].at(i).first == key) {
			pDataSet[setIndex].set(i, make_pair(key, value));
			return;
		}
	}
...
void HashTable<T1, T2>::erase(T1 key) {
	...
	for (int i = 0; i < pDataSet[remIndex].size(); i++) {
		if (pDataSet[remIndex].at(i).first == key) {
...
T2 HashTable<T1, T2>::get(T1 key) {
	...
	for (int i = 0; i < pDataSet[getIndex].size(); i++) {
		const pair<T1, T2> ret_pair = pDataSet[getIndex].at(i);
